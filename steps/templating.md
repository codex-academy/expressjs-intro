---
layout: default
---
# Templates

To make creating views in Express one can use various different template engines; to makes it easy to combine data into html files. We will use Handlebars as our template engine.

## Handlebars

* [Stand-alone Handlebars](/steps/handlebars.html)
* [Handlebars as a view engine](/steps/handlebars_with_express.html)

## Useful links:

* [Loading JSON files using require](https://nodejs.org/api/modules.html#modules_file_modules)
* [Express JS](http://expressjs.com/)
* [Handlebars JS](http://handlebarsjs.com/)
* [Mustache](https://mustache.github.io/)
